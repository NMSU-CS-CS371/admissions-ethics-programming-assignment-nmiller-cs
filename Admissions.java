...
public class Admissions {
    ...
    public static double awareScore(Applicant app) {
        ...
        if (app.income < 40000) score += 0.07;     // low-income boost
        if (app.firstGen) score += 0.07;           // first-generation bonus
        if (app.disability) score += 0.05;         // accessibility consideration
        if (app.dependents) score += 0.07;             // dependency consideration
        if (app.local) score += 0.04;              // local preference
        return Math.min(score, 2.0);               // cap score at 1.0
    }
}